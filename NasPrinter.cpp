...
int NasPrinter::PrintBDF(const std::string &path, const std::string &name,
                         OutputStyle type) {
  ...
  case OutputStyle::multifile: {
    ...
    ofsHeader << "include '" << (name + "_mesh.bdf'")
              << endl; //头文件内部使用相对路径
    ofsHeader << "include '" << (name + "_property.bdf'")
              << endl; //头文件内部使用相对路径
    ofsHeader << "include '" << (name + "_force.bdf'")
              << endl; //头文件内部使用相对路径
...
void NasPrinter::AddCardShort(std::ofstream &ofs,
...
void NasPrinter::AddCardLong(std::ofstream &ofs,
                             const std::vector<std::string> &words) {
  int listNum = 0;
  ofs << std::left;
...
void NasPrinter::AddCardShort(std::stringstream &ofs,
                              const std::vector<std::string> &words) {
  // ofs.str("");//清空缓冲区
  int listNum = 0;
  ofs << std::left;
  ofs << setw(8) << words[0].substr(0, 8);
  ; //第一个关键字
  ofs << std::right;
  for (size_t i = 1; i < words.size() - 1; i++) {
    listNum++;
...
void NasPrinter::AddCardLong(std::stringstream &ofs,
                             const std::vector<std::string> &words) {
  // ofs.str("");//清空缓冲区
  int listNum = 0;
  ofs << std::left;
...
void NasPrinter::addCBAR(C_INT EID, C_INT PID, C_INT GA, C_INT GB, C_DOUBEL X1,
                         C_DOUBEL X2, C_DOUBEL X3) {
  m_print.init(8, 8);
  ...
  AddCardShort(ssMesh, m_print.str_list);
  // AddCardShort(ssMesh, {
  // 	"CBAR", //卡片名 	Printer::int2str(EID),
  // //单元编号 EID
  // 	Printer::int2str(PID),					//单元属性编号
  // PID 	Printer::int2str(GA),					//节点1
  // 	Printer::int2str(GB),					//节点2
  // 	Printer::double2str(X1, 8), //一维单元方向指定 	Printer::double2str(X2,
  // 8),				//一维单元方向指定
...
void NasPrinter::addCQUAD4(C_INT EID, C_INT PID, C_INT G1, C_INT G2, C_INT G3,
                           C_INT G4) {
  m_print.init(7, 8);
...
void NasPrinter::addRBE3(C_INT EID, C_INT REDGRID, C_INT REFC, C_DOUBEL WT1,
                         C_INT C1, C_VECTOR(int) G1j) {
...
void NasPrinter::addPLOAD(C_INT SID, C_DOUBEL P, C_INT G1, C_INT G2, C_INT G3,
                          C_INT G4) {
  m_print.init(7, 8);
...
void NasPrinter::addFORCE(C_INT SID, C_INT G, C_INT CID, C_DOUBEL F,
                          C_DOUBEL N1, C_DOUBEL N2, C_DOUBEL N3) {
  m_print.init(8, 16);
  ...
  // 	Printer::int2str(CID),			//局部坐标系编号
  // 默认可不填=0（全局坐标系） 	Printer::double2str(F , 16),	//集中力的幅值
...
void NasPrinter::addGRAV(C_INT SID, C_INT CID, C_DOUBEL A, C_DOUBEL N1,
                         C_DOUBEL N2, C_DOUBEL N3) {
  m_print.init(7, 16);
...
void NasPrinter::addLOAD(C_INT SID, C_DOUBEL S, C_VECTOR(DoubleIntPair) LOADi) {
  ...
  // {
  // 	strList.push_back(Printer::double2str(LOADi[i].first, 8));
  // //单个载荷的系数 	strList.push_back(Printer::int2str(LOADi[i].second));
  // //单个载荷数据卡编号
  // }
...
void NasPrinter::addPSHELL(const PSHELL &ps) {
  ...
  m_print.add(ps.T);     //厚度
  m_print.add_usize(ps.MID2);
  m_print.add("");
  m_print.add_usize(ps.MID3);
...
void NasPrinter::addEIGRL(C_INT SID, C_INT ND, C_INT MSGLVL) {
  ...
  m_print.add(
      MSGLVL); // Diagnostic Output
               // Level对话层信息，其中0无信息，1每次平移。打印获得的特征值，>1增加对话输出层
  AddCardShort(ssHeader, m_print.str_list);
  // AddCardShort(ssHeader, {
  // 	Printer::int2str(SID),		//集编号（整数>0）
  // 	"","",
  // 	Printer::int2str(ND),		//Number of Desired
  // Roots期望根数（整数>0,或空格） 	Printer::int2str(MSGLVL),	//Diagnostic
  // Output
  // Level对话层信息，其中0无信息，1每次平移。打印获得的特征值，>1增加对话输出层
  // });
}
...
void NasPrinter::addDESVAR(C_INT ID, C_STR LABEL, C_DOUBEL XINIT, C_DOUBEL XLB,
                           C_DOUBEL XUB, C_DOUBEL DELXV) {
  //后续可以提供一个数值检测的集制
  //定义设计变量的卡片
  m_print.init(7, 8);
  ...
  // 	LABEL,
  // //用户指定的设计变量名字 	Printer::double2str(XINIT,8),
  // //设计变量的初始值（double XLB<=XINIT<=XUB） 	Printer::double2str(XLB, 8),
  // //设计变量的下限（double nas默认值-1.0e20） 	Printer::double2str(XUB, 8),
  // //设计变量的上限（double nas默认值+1.0e20） 	Printer::double2str(DELXV,8),
  // //设计变量的该变化量与初始值的比值的最大值（double>0）
  // });
}

void NasPrinter::addDVPREL1(C_INT ID, C_STR TYPE, C_INT PID, C_STR PNAME,
                            C_VECTOR(DoubleIntPair) DVID_COEFi, C_DOUBEL C0,
                            C_DOUBEL PMIN, C_DOUBEL PMAX) {
  ...
  m_print.add(""); //
  m_print.add(DVID_COEFi); // DVIDi DESVAR卡片的ID号（int>0）, COEFi
                           // 线性关系的一次项系数（double）
  AddCardShort(ssHeader, m_print.str_list);
  // vector<string> strList = {
  // 	"DVPREL1",
  ...
  // 	TYPE,
  // //属性类型的名字，如“PBAR”，“PBEAM”等（string） 	Printer::int2str(PID),
  // //属性的ID号（int>0） 	PNAME,
  // //属性名字，如“T”、“A”，或属性卡片字域（Field）的未知或者单元属性表中字符（Word）的位置（sring或int>0）
  // 	Printer::double2str(PMIN, 8),
  // //该属性允许的最小值（double，nas默认值-1.0e20） 	Printer::double2str(PMAX,
  // 8),	//该属性允许的最大值（double，nas默认值+1.0e20）
  // 	Printer::double2str(C0, 8),
  // //线性关系的常数项（double，nas默认值为0.0）
  // 	"",
  // };
  // strList.reserve(strList.size() + DVID_COEFi.size());
  // for (size_t i = 0; i < DVID_COEFi.size(); i++)
  // {
  // 	strList.push_back(Printer::int2str(DVID_COEFi[i].second));//DVIDi
  // DESVAR卡片的ID号（int>0）
  // 	strList.push_back(Printer::double2str(DVID_COEFi[i].first, 8));//COEFi
  // 线性关系的一次项系数（double）
  // }
  // AddCardShort(ssHeader, strList);
}

void NasPrinter::addDRESP1(C_INT ID, C_STR LABLE, C_STR RTYPE, C_STR PTYPE,
                           C_INT REGION, C_INT ATTA, C_INT ATTB,
                           C_VECTOR(int) ATTi) {
...
void NasPrinter::addDRESP1_STRESS(C_INT ID, C_STR LABEL, C_STR PTYPE,
                                  C_INT ATTA, C_VECTOR(int) ATTi) {
  addDRESP1(ID, LABEL, "STRESS", PTYPE, -1, ATTA, -1, ATTi);
}

void NasPrinter::addDCONSTR(C_INT DCID, C_INT RID, C_DOUBEL LALLOW,
                            C_DOUBEL UALLOW) {
  //定义约束
  m_print.init(5, 8);
  ...
  // 	Printer::int2str(RID),
  // //设计响应（DRESPi卡片）定义的ID号（int>0） 	Printer::double2str(LALLOW, 8),
  // //响应量的下限（double nas默认值+1.0e20） 	Printer::double2str(UALLOW, 8),
  // //响应量的上限（double nas默认值+1.0e20）
  // });
}
