...
void CSTsurface::CST3D()
{
	...
	{ return 0; }; //{ return x*x * Xlength * 0.31; };
	...
	int id = 0;
	for (size_t i = 1; i < NFaiU; i++)
	{
		for (size_t j = 1; j < NEta; j++)
		{
...
void CSTsurface::EvalCurvature()
{
	...
	for (size_t i = 0; i < NFaiU; i++)
	{
		for (size_t j = 0; j < NEta - 1; j++)
		{
...
void CSTsurface::EvalGrid()
{
	...
	for (size_t ix = 0; ix < NEta - 1; ix++)
	{
		double area = 0;
		for (size_t iu = 0; iu < NFaiU - 1; iu++)
		{
			area += (-1) * PUPP.Z(iu, ix) * PUPP.Y(iu + 1, ix) - (-1) * PUPP.Z(iu + 1, ix) * PUPP.Y(iu, ix);
			SurfaceAeraBody = SurfaceAeraBody + (sqrt(pow(PUPP.Z(iu, ix) - PUPP.Z(iu + 1, ix), 1) + pow(PUPP.Y(iu, ix) - PUPP.Y(iu + 1, ix), 1)) + sqrt(pow(PUPP.Z(iu, ix + 1) - PUPP.Z(iu + 1, ix + 1), 1) + pow(PUPP.Y(iu, ix + 1) - PUPP.Y(iu + 1, ix + 1), 1))) / 2 * (PUPP.X(0, ix + 1) - PUPP.X(0, ix));
			planformarea = planformarea + (PUPP.X(0, ix + 1) - PUPP.X(0, ix)) * abs(PUPP.Z(iu + 1, ix) - PUPP.Z(iu, ix));
		}
		for (size_t il = NFaiL - 1; il > 0; il--)
		{
			...
			SurfaceAeraBody = SurfaceAeraBody +
							  (sqrt(pow(PLOW.Z(il, ix) - PLOW.Z(il - 1, ix), 1) + pow(PLOW.Y(il, ix) - PLOW.Y(il - 1, ix), 1)) +
							   sqrt(pow(PLOW.Z(il, ix + 1) - PLOW.Z(il - 1, ix + 1), 1) + pow(PLOW.Y(il, ix + 1) - PLOW.Y(il - 1, ix + 1), 1))) /
								  2 * (PLOW.X(0, ix + 1) - PLOW.X(0, ix));
		}
...
{
	Meshing mesh;
	CBlockInfo info = CBlockInfo(0.3, 1, 0.3, 1);
	// CBlockInfo info = CBlockInfo(1, NEta-1, 1, NFaiU-1);
	// CNomalInfo info_n = CNomalInfo(NEta, NFaiU);

	// Meshing m1,m2;
	// m1.calc_mesh(info);
	// m2.calc_mesh(info_n);
	// cout << m1.eta_mesh << endl;
	// cout << m2.eta_mesh << endl;
	// cout << m1.fai_mesh - m2.fai_mesh << endl;
	if (ifrefresh) //??????
	{
		mesh.calc_mesh(CBlockInfo(eta_block_size,n_eta, fai_u_block_size,n_fai_u));
		FaiU = mesh.fai_mesh;
		EtaU = mesh.eta_mesh;
		NEta = mesh.eta_num;
		NFaiU = mesh.fai_num;
		mesh.calc_mesh(CBlockInfo(eta_block_size,n_eta, fai_l_block_size,n_fai_l));
		FaiL = mesh.fai_mesh;
		EtaL = mesh.eta_mesh;
		NEta = mesh.eta_num;
		NFaiL = mesh.fai_num;	
	...
	else //?????????о??????(?????????????????????????????????????)
	{
		...
		{
			// mesh.calc_mesh(CNomalInfo(NEta, NFaiL));
			mesh.calc_mesh(CBlockInfo(eta_block_size,n_eta, fai_l_block_size,n_fai_l));
			FaiL = mesh.fai_mesh;
			EtaL = mesh.eta_mesh;
			NEta = mesh.eta_num;
...
mat CSTsurface::MakeBone(double bias1, double bias2) //(?????bias? - 1to1)
{
	...
	for (int i = 0; i < numX; i++) //??????????????????
	{
		...
		{
			if (bias1 < 0)
				delta1 = (zc - PLOW.Z(0, i)) * bias1;
			else
				delta1 = (PLOW.Z((numZlow - 1), i) - zc) * bias1;
		}
		if (i == numX - 1)
		{
			if (bias2 < 0)
				delta2 = (zc - PLOW.Z(0, i)) * bias2;
			else
				delta2 = (PLOW.Z((numZlow - 1), i) - zc) * bias2;
		}
	...
	if (X.n_elem != 0 && Z.n_elem != 0)
	{
		...
		for (int j = 0; j < numj; j++)
		{
			...
			if (lenZ > 0)
			{
				...
				mat temp;
				interp1(siteZ, PLOWXm.col(j), Z, temp);
				PLOWX.col(j) = temp;
				interp1(siteZ, PUPPXm.col(j), Z, temp);
				PUPPX.col(j) = temp;
				interp1(siteZ, PLOWYm.col(j), Z, temp);
				PLOWY.col(j) = temp;
				interp1(siteZ, PUPPYm.col(j), Z, temp);
				PUPPY.col(j) = temp;
				interp1(siteZ, PLOWZm.col(j), Z, temp);
				PLOWZ.col(j) = temp;
				interp1(siteZ, PUPPZm.col(j), Z, temp);
				PUPPZ.col(j) = temp;
			}
			else
			{
...
int CSTsurface::MakeStruct1D(mat X, mat Z)
{
	...
	{
		elem_h(i) = (pt_h((size_t)elem_aero(i, 0), 0) +
					 pt_h((size_t)elem_aero(i, 1), 0) +
					 pt_h((size_t)elem_aero(i, 2), 0) +
					 pt_h((size_t)elem_aero(i, 3), 0)) /
					4;
	}
